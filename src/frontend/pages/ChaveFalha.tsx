...
const ChaveFalha = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      setError(null)
      const response = await apiService.chaveFalha(selectedWallet)
      setResult(response)
    } catch (err: any) {
  ...
  return (
    <Container>
      <Row className="justify-content-md-center">
        <Col md={8}>
          <h1 className="mb-4">Consulta de Chave - Falha</h1>
          <Card>
            <Card.Body>
              <Form onSubmit={handleSubmit}>
                ...
                >
                  {loading ? (
                    ...
                    'Executar Consulta de Chave (Falha)'
                  )}
                </Button>
              </Form>
              ...
              {result && (
                <Alert 
                  variant={result.message.includes('não encontrada') ? 'danger' : 'success'} 
                  className="mt-3"
                >
                  <h5>Resultado:</h5>
