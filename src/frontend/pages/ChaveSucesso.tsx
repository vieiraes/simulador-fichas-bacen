...
const ChaveSucesso = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      setError(null)
      const response = await apiService.chaveSucesso(selectedWallet)
      setResult(response)
    } catch (err: any) {
      setError(err.response?.data?.message || 'Erro ao processar consulta de chave')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Container>
      <Row className="justify-content-md-center">
        <Col md={8}>
          <h1 className="mb-4">Consulta de Chave - Sucesso</h1>
          <Card>
            <Card.Body>
              <Form onSubmit={handleSubmit}>
                ...
                >
                  {loading ? (
                    ...
                    'Executar Consulta de Chave (Sucesso)'
                  )}
                </Button>
              </Form>
              ...
              {result && (
                <Alert 
                  variant={result.message.includes('sucesso') ? 'success' : 'danger'} 
                  className="mt-3"
                >
                  <h5>Resultado:</h5>
                  <p><strong>Mensagem:</strong> {result.message}</p>
                  {result.wallet && (
                    <p><strong>Bucket Cliente:</strong> {result.wallet.balde} fichas</p>
                  )}
